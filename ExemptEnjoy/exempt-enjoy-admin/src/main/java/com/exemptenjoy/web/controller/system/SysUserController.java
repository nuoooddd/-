...
import com.exemptenjoy.common.enums.BusinessType;
...
import com.exemptenjoy.system.service.ISysDeptService;

import com.exemptenjoy.system.service.ISysRoleService;
...
public class SysUserController extends BaseController
{
    ...
    @PreAuthorize("@ss.hasPermi('system:user:query')")
    @GetMapping("/enterpriseBindings")
    public AjaxResult enterpriseBindings()
    {
        List<EeTargetData> allTargets = targetDataMapper.selectEeTargetDataList(new EeTargetData());
        Map<Long, List<String>> bindings = new HashMap<>();
        for (EeTargetData target : allTargets) {
            if (target.getUserId() != null) {
                bindings.computeIfAbsent(target.getUserId(), k -> new ArrayList<>())
                        .add(target.getTargetName());
            }
        }
