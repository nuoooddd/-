  ...
  <div class="dashboard-page">
    ...
    <div class="stat-cards">
      <div class="stat-card" v-for="s in statCards" :key="s.key" @click="s.route && $router.push(s.route)">
        <div class="sc-icon" :style="{background: s.bgColor}"><el-icon :size="18"><component :is="s.icon" /></el-icon></div>
        <div class="sc-info">
    ...
    <div class="section-card">
          <div class="sec-header">
            <h3>五步闭环流转</h3>
            <el-button text type="primary" size="small" @click="$router.push('/biz/matchRecord')">查看全部 <el-icon><ArrowRight /></el-icon></el-button>
          </div>
          <div class="pipeline">
          ...
          <div v-for="r in recentRecords.slice(0,5)" :key="r.recordId" class="record-card">
            <div class="rc-top"><span class="rc-name">{{ r.policyName }}</span><el-tag :type="statusTag(r.status)" size="small">{{ statusLabel(r.status) }}</el-tag></div>
            <div class="rc-bottom"><span>{{ r.targetName }}</span><span class="rc-money">{{ formatMoney(r.fundAmount) }}</span><el-tag :type="riskTag(r.riskLevel)" size="small">{{ riskLabel(r.riskLevel) }}</el-tag></div>
          </div>
        </div>
        ...
        <div class="section-card">
          <div class="sec-header">
            <h3>风控概览</h3>
            <el-button text type="primary" size="small" @click="$router.push('/biz/matchRecord')">详情 <el-icon><ArrowRight /></el-icon></el-button>
          </div>
          <div class="risk-row">
    ...
    <div class="section-card">
          <div class="sec-header">
            <h3>资金池预算</h3>
            <el-button text type="primary" size="small" @click="$router.push('/biz/fund')">管理 <el-icon><ArrowRight /></el-icon></el-button>
          </div>
        ...
        <div class="section-card">
          <div class="sec-header">
            <h3>政策到期预警</h3>
            <el-button text type="primary" size="small" @click="$router.push('/biz/policy')">管理 <el-icon><ArrowRight /></el-icon></el-button>
          </div>
...
.dashboard-page { padding: 12px; background: #f5f7fa; min-height: 100%; }

.page-header { margin-bottom: 12px; h2 { font-size: 18px; font-weight: 700; color: #0f172a; margin: 0 0 2px; } p { font-size: 12px; color: #64748b; margin: 0; } }

.stat-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 12px; }
.stat-card { display: flex; align-items: center; gap: 8px; padding: 10px 12px; background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; cursor: pointer; }
.sc-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #0052ff; flex-shrink: 0; }
.sc-value { font-size: 16px; font-weight: 700; color: #0f172a; font-family: "Outfit", sans-serif; }
.sc-label { font-size: 10px; color: #64748b; margin-top: 1px; }
.sc-arrow { margin-left: auto; color: #cbd5e1; font-size: 12px; }

.section-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 14px; margin-bottom: 10px; }
.sec-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; h3 { font-size: 14px; font-weight: 600; color: #0f172a; margin: 0; } }

.pipeline { display: flex; align-items: center; margin-bottom: 12px; gap: 0; }
.pipe-step { display: flex; align-items: center; gap: 4px; flex: 1; &.active .ps-dot { box-shadow: 0 0 0 2px rgba(0,82,255,0.15); } }
.ps-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; opacity: 0.4; }
.pipe-step.active .ps-dot { opacity: 1; }
.ps-name { font-size: 10px; color: #64748b; font-weight: 500; }
.pipe-step.active .ps-name { color: #0f172a; }
.ps-count { font-size: 10px; color: #94a3b8; }
.ps-line { flex: 1; height: 2px; background: #e2e8f0; margin: 0 2px; min-width: 8px; }

.record-card { padding: 8px 10px; background: #f8fafc; border-radius: 8px; margin-bottom: 6px; border: 1px solid #e2e8f0; }
.rc-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }
.rc-name { font-size: 12px; font-weight: 600; color: #1e293b; flex:1; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-right:6px; }
.rc-bottom { display: flex; align-items: center; gap: 8px; font-size: 11px; color: #64748b; }
.rc-money { color: #0052ff; font-weight: 600; font-family: Consolas, monospace; }

.compare-item { padding: 10px; border-radius: 8px; margin-bottom: 8px; }
.compare-item.old { background: #fef2f2; border: 1px solid #fecaca; }
.compare-item.new { background: #f0fdf4; border: 1px solid #bbf7d0; }
.ci-label { font-size: 11px; font-weight: 600; margin-bottom: 4px; }
...
.ci-num { font-size: 20px; font-weight: 700; font-family: "Outfit", sans-serif; }
.compare-item.old .ci-num { color: #dc2626; }
.compare-item.new .ci-num { color: #16a34a; }
.ci-unit { font-size: 11px; color: #64748b; }
.ci-desc { font-size: 10px; color: #94a3b8; margin-top: 2px; }

.exempt-rate { margin-top: 8px; }
.er-label { font-size: 11px; color: #64748b; display: block; margin-bottom: 4px; }

.risk-row { display: flex; gap: 8px; }
.risk-item { flex: 1; text-align: center; padding: 10px; border-radius: 8px; }
...
.ri-count { font-size: 20px; font-weight: 700; font-family: "Outfit", sans-serif; }
.risk-item.low .ri-count { color: #16a34a; }
.risk-item.mid .ri-count { color: #d97706; }
.risk-item.high .ri-count { color: #dc2626; }
.ri-label { font-size: 10px; color: #64748b; margin-top: 2px; }

.fund-row { margin-bottom: 10px; }
.fr-head { display: flex; justify-content: space-between; margin-bottom: 4px; }
.fr-name { font-size: 12px; font-weight: 500; color: #334155; }
.fr-amount { font-size: 11px; color: #94a3b8; font-family: Consolas, monospace; }

.expire-list { display: flex; flex-direction: column; gap: 6px; }
.expire-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 10px; border-radius: 8px; background: #f8fafc; border: 1px solid #e2e8f0; }
.expire-row.urgent { background: #fef2f2; border-color: #fecaca; }
.er-left { display: flex; align-items: center; gap: 6px; font-size: 12px; color: #334155; }
.er-right { display: flex; align-items: center; gap: 8px; }
