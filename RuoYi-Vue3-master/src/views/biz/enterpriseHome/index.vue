  ...
  <div class="ep-home">
    <div class="ep-hero">
      <div class="hero-decor">
        <div class="decor-ring r1"></div>
        <div class="decor-ring r2"></div>
        <div class="decor-ring r3"></div>
      </div>
      <div class="hero-body">
        <div class="hero-left">
          ...
          <p class="hero-sub">智能匹配惠企政策 · 补贴直达银行账户 · 零申请零跑腿</p>
          <div class="hero-actions">
            <el-button type="primary" size="large" round @click="$router.push('/portal/userFulfill')"><el-icon><Money /></el-icon> 我的兑付</el-button>
            <el-button size="large" round plain @click="showEditDialog=true"><el-icon><Edit /></el-icon> 完善企业画像</el-button>
          </div>
        </div>
        <div class="hero-right">
          <div class="metric-ring" v-for="m in heroMetrics" :key="m.key" :class="'ring-'+m.key">
            <div class="ring-value">{{ m.value }}</div>
            <div class="ring-label">{{ m.label }}</div>
            <svg class="ring-svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="42" fill="none" stroke="rgba(255,255,255,0.15)" stroke-width="6"/><circle cx="50" cy="50" r="42" fill="none" :stroke="m.color" stroke-width="6" stroke-linecap="round" :stroke-dasharray="m.dash" :stroke-dashoffset="m.offset" transform="rotate(-90 50 50)"/></svg>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog title="完善企业画像" v-model="showEditDialog" width="700px" append-to-body destroy-on-close>
...
const heroMetrics = computed(() => {
  ...
  const total = Math.max(d.matchedCount || 0, 1)
  const circ = 2 * Math.PI * 42
  return [
    { key:'pushed', label:'待确认', value: d.pushedCount||0, color:'#f59e0b', dash:circ, offset:circ - circ * Math.min((d.pushedCount||0)/total, 1) },
    { key:'matched', label:'已匹配', value: d.matchedCount||0, color:'#3b82f6', dash:circ, offset:circ - circ * Math.min((d.matchedCount||0)/total, 1) },
    { key:'fulfilled', label:'已兑付', value: d.fulfilledCount||0, color:'#10b981', dash:circ, offset:circ - circ * Math.min((d.fulfilledCount||0)/total, 1) },
    { key:'amount', label:'兑付额(万)', value: formatMoneyShort(d.fulfilledAmount||0), color:'#8b5cf6', dash:circ, offset:0 }
  ]
})
...
.ep-home { background:#f0f4f8; min-height:calc(100vh - 84px); }

.ep-hero {
  position:relative; padding:48px 40px 36px; overflow:hidden;
  background:linear-gradient(135deg, #eff6ff 0%, #dbeafe 50%, #e0f2fe 100%);
  .hero-decor { position:absolute; inset:0; pointer-events:none; }
  .decor-ring { position:absolute; border-radius:50%; border:1px solid rgba(59,130,246,0.08); }
  .r1 { width:500px; height:500px; top:-200px; right:-100px; }
  .r2 { width:350px; height:350px; bottom:-120px; left:-60px; }
  .r3 { width:200px; height:200px; top:50%; left:60%; border-color:rgba(59,130,246,0.12); }
  .hero-body { position:relative; z-index:1; display:flex; align-items:center; justify-content:space-between; max-width:1400px; margin:0 auto; }
  .hero-left { flex:1; }
  .hero-badge { display:inline-flex; align-items:center; gap:6px; padding:6px 16px; border-radius:20px; background:rgba(59,130,246,0.1); border:1px solid rgba(59,130,246,0.2); color:#2563eb; font-size:13px; margin-bottom:16px; }
  .hero-title { font-size:28px; font-weight:700; color:#0f172a; margin:0 0 10px; }
  .hl { color:#2563eb; }
  .hero-sub { font-size:15px; color:#64748b; margin:0 0 24px; }
  .hero-actions { display:flex; gap:12px; }
  .hero-right { display:flex; gap:20px; }
  .metric-ring { position:relative; width:100px; height:100px; text-align:center; display:flex; flex-direction:column; align-items:center; justify-content:center; }
  .ring-svg { position:absolute; inset:0; width:100%; height:100%; }
  .ring-value { font-size:20px; font-weight:700; color:#0f172a; font-family:'Outfit',sans-serif; position:relative; z-index:1; }
  .ring-label { font-size:10px; color:#64748b; position:relative; z-index:1; margin-top:2px; }
}

.ep-body { padding:24px 40px; max-width:1400px; margin:0 auto; }
...
.section-card {
  ...
  .sec-head h3 { font-size:16px; font-weight:600; color:#1e293b; margin:0; display:flex; align-items:center; gap:8px; }
}

.todo-list { display:flex; flex-direction:column; gap:12px; }
.todo-item { display:flex; align-items:center; justify-content:space-between; padding:14px 16px; background:#f8fafc; border-radius:10px; border:1px solid #e8ecf1; transition:all 0.2s; }
.todo-item:hover { border-color:#cbd5e1; box-shadow:0 2px 8px rgba(0,0,0,0.04); }
.todo-left { display:flex; align-items:center; gap:14px; flex:1; min-width:0; }
.todo-step { width:38px; height:38px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:18px; color:#fff; flex-shrink:0; }
...
.match-item { border:1px solid #e8ecf1; border-radius:12px; overflow:hidden; cursor:pointer; transition:all 0.2s; }
.match-item:hover { border-color:#bfdbfe; }
.match-item.open { border-color:#93c5fd; box-shadow:0 2px 8px rgba(59,130,246,0.08); }
...
.mi-meta { font-size:11px; color:#94a3b8; flex-shrink:0; }
.mi-arrow { color:#94a3b8; transition:transform 0.2s; }
.mi-arrow.open { transform:rotate(180deg); color:#3b82f6; }
.mi-detail { padding:0 16px 16px; border-top:1px solid #f1f5f9; }
.mi-rule { background:#f8fafc; border-radius:10px; padding:12px 14px; margin-top:10px; }
.mr-head { display:flex; align-items:center; gap:8px; margin-bottom:8px; flex-wrap:wrap; }
.mr-name { font-size:12px; font-weight:600; color:#334155; }
.mr-expr { font-size:11px; color:#94a3b8; font-family:'Courier New',monospace; background:#f1f5f9; padding:2px 6px; border-radius:4px; }
.mr-conds { display:flex; flex-direction:column; gap:6px; }
.mr-cond { display:flex; align-items:center; gap:8px; padding:6px 10px; background:#fff; border-radius:8px; border:1px solid #f1f5f9; flex-wrap:wrap; font-size:12px; }
.mc-field { font-weight:600; color:#334155; background:#eff6ff; padding:1px 6px; border-radius:4px; font-family:'Courier New',monospace; }
.mc-op { color:#64748b; font-family:'Courier New',monospace; }
.mc-exp { color:#6366f1; font-weight:500; font-family:'Courier New',monospace; }
...
.pf-row { display:flex; justify-content:space-between; padding:8px 12px; background:#f8fafc; border-radius:8px; }
.pf-k { font-size:12px; color:#94a3b8; }
...
.pf-cell { padding:8px 10px; background:#f8fafc; border-radius:8px; }
.pf-ck { font-size:10px; color:#94a3b8; display:block; margin-bottom:2px; }
...
.profile-guide h3 { font-size:18px; font-weight:700; color:#1e293b; margin:0; }
...
.pol-item { display:flex; align-items:stretch; border-radius:10px; overflow:hidden; border:1px solid #e8ecf1; transition:all 0.2s; background:#fff; }
.pol-item:hover { border-color:#bfdbfe; box-shadow:0 2px 8px rgba(59,130,246,0.06); }
.pol-bar { width:4px; flex-shrink:0; }
